...
export const regenerateStory = inngest.createFunction(
  { id: "regenerate-story" },
  { event: "telly/regenerate.story" },
  async ({ event, step }) => {
    const userId = event.data.userId;
    if (!userId) throw new Error("User not authenticated");
    if (!event.data.storyId) throw new Error("Story ID is required");
    ...
    const news = await step.run("get-news", async () => {
      ...
      const response = await openai.responses.create({
        model: "gpt-5",
        tools: [{ type: "web_search" }],
        input: `Latest news about ${event.data.prompt}.`,
      });
